...
const sbClient =
  window.supabase?.createClient
    ? window.supabase.createClient(
        SPACEBOI_SUPABASE_URL,
        SPACEBOI_SUPABASE_KEY,
        {
          auth: {
            storageKey: "shot-by-spaceboi-client-auth"
          }
        }
      )
    : null;
...
function bookingShootKey(booking) {
  const slug = booking?.shoot_types?.slug || "";
  const slugMap = {
    "portrait-birthday": "birthday",
    "studio-creative": "lifestyle",
    "graduation": "graduation",
    "maternity": "maternity",
    "brand-product": "brand",
    "wedding-event": "events",
    "other": "custom"
  };
...
function inferBookingPackage(booking) {
  const savedName = booking?.package_name || "";
  const savedPrice = Number(booking?.package_price || 0);

  const shootKey = bookingShootKey(booking);
  const shoot = shootKey ? shootData[shootKey] : null;

  let matched = null;

  if (shoot?.packages?.length) {
    matched = shoot.packages.find(pkg => {
      const durationMatches =
        parseDurationMinutes(pkg.duration) === Number(booking.duration_minutes || 0);

      const pkgPhotos = numericPrefix(pkg.photos);
      const pkgEdited = numericPrefix(pkg.edited);

      const photosMatch =
        pkgPhotos === null ||
        booking.photos_received === null ||
        Number(booking.photos_received) === pkgPhotos;

      const editedMatch =
        pkgEdited === null ||
        booking.edited_photos === null ||
        Number(booking.edited_photos) === pkgEdited;

      return durationMatches && photosMatch && editedMatch;
    });
  }

  return {
    name: savedName || matched?.label || "Photography Package",
    price:
      savedPrice > 0
        ? savedPrice
        : parsePackagePrice(matched?.price || 0),
    displayPrice:
      savedPrice > 0
        ? money(savedPrice)
        : (matched?.price || money(0)),
    photos: booking.photos_received ?? numericPrefix(matched?.photos),
    edited: booking.edited_photos ?? numericPrefix(matched?.edited),
...
function prettyInvoiceNumber(invoice) {
  if (invoice?.invoice_number && /^SB-\d{3,}$/i.test(invoice.invoice_number)) {
    return invoice.invoice_number.toUpperCase();
  }

  return invoice?.invoice_number || "SB-001";
}

let invoiceLogoDataUrlCache = null;

async function getInvoiceLogoDataUrl() {
  if (invoiceLogoDataUrlCache) return invoiceLogoDataUrlCache;

  try {
    const response = await fetch("spaceboi-logo.png", { cache: "force-cache" });
    if (!response.ok) throw new Error("Logo could not be loaded.");

    const blob = await response.blob();

    invoiceLogoDataUrlCache = await new Promise((resolve, reject) => {
    ...
    console.warn("Invoice logo skipped:", error);
    return null;
  }
}
  ...
  bookingForm.addEventListener("submit", async event => {
    ...
    const { data: booking, error: bookingError } = await sbClient
      ...
      .insert({
        ...
        package_price: parsePackagePrice(pkg.price),
        quoted_subtotal: parsePackagePrice(pkg.price),
        final_total: parsePackagePrice(pkg.price),
...
function invoicePdfName(invoice) {
  return `${prettyInvoiceNumber(invoice)}.pdf`;
}

async function downloadInvoicePdf(invoice, booking) {
  ...
  const plum = [45, 21, 54];
  const plumSoft = [74, 36, 87];
  const gold = [195, 154, 75];
  const champagne = [244, 235, 222];
  const ink = [41, 34, 41];
  const muted = [115, 106, 115];
  ...
  const packageInfo = inferBookingPackage(booking);

  const subtotal =
    Number(invoice.subtotal || 0) > 0
      ? Number(invoice.subtotal)
      : Number(booking.quoted_subtotal || 0) > 0
        ? Number(booking.quoted_subtotal)
        : Number(packageInfo.price || 0);

  const travelFee = Number(invoice.travel_fee || booking.travel_fee || 0);
  const extras = Number(invoice.extras_total || booking.extras_total || 0);
  const discount = Number(invoice.discount_amount || booking.discount_amount || 0);

  const calculatedTotal = Math.max(0, subtotal + travelFee + extras - discount);
  const total =
    Number(invoice.total || 0) > 0
      ? Number(invoice.total)
      : Number(booking.final_total || 0) > 0
        ? Number(booking.final_total)
        : calculatedTotal;

  const paid = Number(invoice.amount_paid || booking.amount_paid || 0);
  const balance =
    Number(invoice.balance || 0) > 0
      ? Number(invoice.balance)
      : Math.max(0, total - paid);

  const invoiceNumber = prettyInvoiceNumber(invoice);
  const issueDate = new Date(invoice.created_at || Date.now()).toLocaleDateString("en-JM", {
    year: "numeric",
    month: "long",
    day: "numeric"
  });
  ...
  const logo = await getInvoiceLogoDataUrl();
  if (logo) {
    try {
      doc.addImage(logo, "PNG", left, 8, 48, 30, undefined, "FAST");
    } catch (error) {
      console.warn("Invoice logo rendering skipped:", error);
    }
  } else {
    doc.setTextColor(...white);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(14);
    doc.text("SHOT BY SPACEBOI", left, 24);
  }

  doc.setTextColor(...white);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(19);
  doc.text(invoice.invoice_type === "final" ? "FINAL INVOICE" : "BOOKING INVOICE", right, 18, {
    align: "right"
  });

  doc.setFont("helvetica", "normal");
  doc.setFontSize(8.5);
  doc.setTextColor(234, 220, 200);
  doc.text(invoiceNumber, right, 26, { align: "right" });
  doc.text(issueDate, right, 32, { align: "right" });

  let y = 61;

  // Client + shoot summary
  doc.setTextColor(...gold);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7.5);
  doc.text("BILLED TO", left, y);
  doc.text("SESSION", 111, y);

  y += 7;
  doc.setTextColor(...ink);
  doc.setFontSize(11);
  doc.text(activeAccountProfile.fullName || "Client", left, y);
  doc.text(sessionName, 111, y);

  y += 5;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8.5);
  doc.setTextColor(...muted);
  doc.text(activeAccountProfile.email || "", left, y);
  doc.text(
    `${formatBookingDate(booking.preferred_date)} • ${formatBookingTime(booking.preferred_time)}`,
    111,
    y
  );
  ...
  if (booking.location_type) {
    doc.text(
      booking.location_type === "On Location" && booking.location_text
        ? `On Location • ${booking.location_text}`
  ...
  doc.roundedRect(left, y, 174, 38, 3, 3, "F");

  doc.setTextColor(...plum);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7.5);
  doc.text("PACKAGE DETAILS", left + 6, y + 8);

  doc.setFontSize(12);
  doc.text(packageInfo.name, left + 6, y + 17);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(8.5);
  doc.setTextColor(...plumSoft);

  const packageLine = [
    packageInfo.duration,
    packageInfo.photos ? `${packageInfo.photos} photos` : null,
    packageInfo.edited ? `${packageInfo.edited} edited` : null
  ].filter(Boolean).join("  •  ");

  doc.text(packageLine || "Photography package", left + 6, y + 25);

  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.setTextColor(...plum);
  doc.text(money(subtotal), right - 6, y + 17, { align: "right" });
  ...
  rows.forEach(([label, value]) => {
    ...
    doc.setTextColor(...ink);
    doc.text(
      Number(value) < 0 ? `-${money(Math.abs(value))}` : money(value),
      right,
      y,
      { align: "right" }
    );
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...muted);
  doc.text(invoice.invoice_type === "final" ? "Amount paid" : "Deposit paid", left, y);
  doc.setTextColor(...ink);
  doc.text(money(paid), right, y, { align: "right" });

  y += 8;
  doc.setFillColor(...plum);
  doc.roundedRect(left, y - 5, 174, 15, 2, 2, "F");

  doc.setTextColor(...white);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text(balance <= 0 ? "BALANCE PAID" : "BALANCE DUE", left + 5, y + 4);
  doc.text(money(balance), right - 5, y + 4, { align: "right" });
  ...
  doc.setTextColor(...plum);
  doc.setFontSize(10);
  doc.text(invoice.status || "Deposit Paid", left, y);
  ...
  doc.text(
    invoice.invoice_type === "final"
      ? "Thank you for choosing Shot by Spaceboi."
      : "Deposit payments are non-refundable. Any remaining balance is shown above.",
    left,
    y
  );
  ...
  // Footer
  doc.setFillColor(...plum);
  doc.rect(0, 279, pageW, 18, "F");

  doc.setTextColor(234, 220, 200);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7);
  doc.text("shotbyspaceboi@gmail.com", left, 288);
  doc.text("(876) 481-1489", pageW / 2, 288, { align: "center" });
  doc.text("2 Majestic Pen Road", right, 288, { align: "right" });
...
async function uploadPaymentProof(user, booking, file, amount, paymentType = "deposit") {
...
function attachPaymentProofHandler(card, booking, user) {
  ...
  form.addEventListener("submit", async event => {
    ...
    const paymentType = form.dataset.paymentType || "deposit";
    ...
    try {
      await uploadPaymentProof(user, booking, file, amount, paymentType);
...
function attachBookingChangeHandlers(card, booking, user) {
  const reschedule = card.querySelector("[data-request-reschedule]");
  const cancel = card.querySelector("[data-request-cancel]");

  reschedule?.addEventListener("click", async () => {
    const date = prompt("What new date would you like? Use YYYY-MM-DD.");
    if (!date) return;
    const time = prompt("What new time would you like? Use HH:MM, for example 14:30.");
    if (!time) return;
    const note = prompt("Optional note for Spaceboi:") || "";

    const { error } = await sbClient.rpc("submit_booking_change_request", {
      p_booking_id: booking.id,
      p_type: "reschedule",
      p_date: date,
      p_time: time,
  ...
  cancel?.addEventListener("click", async () => {
    if (!confirm("Send a cancellation request for this booking?")) return;
    const note = prompt("Optional note for Spaceboi:") || "";
...
function attachClientReviewHandler(card, booking, user) {
  const form = card.querySelector(".client-review-form");
  if (!form) return;
  ...
  form.addEventListener("submit", async event => {
    ...
    const { error } = await sbClient.from("reviews").insert({
      ...
      client_id: user.id,
      client_name: activeAccountProfile.fullName || "Client",
      rating,
      body,
      approved: false
    });
...
async function loadMyBookings(user) {
  ...
  const { data: myReviews } = await sbClient
    .from("reviews")
    .select("booking_id")
    .eq("client_id", user.id);
  ...
  uniqueBookings.forEach(booking => {
    ...
    const balance =
      Math.max(0, effectiveTotal - Number(booking.amount_paid || 0));
    ...
    const hasSubmittedPayment = payments.some(payment => payment.status === "submitted");
    const depositAccepted = payments.some(payment => payment.payment_type === "deposit" && payment.status === "accepted");
    const balanceAccepted = payments.some(payment => payment.payment_type === "balance" && payment.status === "accepted");
    ...
    const canUploadPayment = canUploadDeposit || canUploadBalance;
    const paymentMode = canUploadBalance ? "balance" : "deposit";
    const paymentHeading = canUploadBalance ? "Balance Payment" : "Deposit Payment";

    const suggestedPaymentAmount = canUploadBalance
      ? balance
      : (Number(booking.deposit_required || 0) > 0 ? Number(booking.deposit_required) : "");
    ...
    card.innerHTML = `
      <div class="account-booking-top">
        <div>
          ...
          <p class="booking-package-line">${escapeHtml(packageInfo.name)} • ${money(effectiveTotal)}</p>
        </div>
      ...
      ${["confirmed","booked"].includes(booking.status) || effectivePaymentStatus !== "none"
        ? `
          <div class="client-payment-panel">
            ...
            ${hasSubmittedPayment
              ? `<p class="payment-state-copy">Your proof has been submitted and is waiting for review.</p>`
              : canUploadPayment
                ? `
                  <div class="payment-instructions-box">
                    <p class="payment-instructions-title">${escapeHtml(paymentInstructions.heading || "Payment Instructions")}</p>
                    ${renderPaymentInstructions(paymentInstructions)}
                  </div>
                `
                : balanceAccepted || balance <= 0
                  ? `<p class="payment-state-copy success">Your payments are up to date.</p>`
                  : depositAccepted
                    ? `<p class="payment-state-copy success">Your deposit has been accepted and your booking is secured.</p>`
                    : effectivePaymentStatus === "rejected"
                      ? `<p class="payment-state-copy rejected">The last proof was rejected${latestPayment?.admin_note ? `: ${escapeHtml(latestPayment.admin_note)}` : "."}</p>`
                      : ""
            }

            ${canUploadPayment
              ? `
                <form class="payment-proof-form" data-payment-type="${paymentMode}">
                  <label>
                    Amount paid (JMD)
                    <input
                      name="paymentAmount"
                      ...
                      min="1"
                      step="1"
                      value="${suggestedPaymentAmount}"
                      placeholder="${suggestedPaymentAmount ? "" : "Enter amount paid"}"
                      required
                    >
                  </label>
                  <label>
                  ...
                  <button class="gold-btn full-width" type="submit">
                    ${canUploadBalance ? "UPLOAD BALANCE PROOF" : "UPLOAD DEPOSIT PROOF"}
                  </button>
                </form>
              `
              : ""
      ...
      ${["confirmed","booked"].includes(booking.status)
        ? `
          <div class="client-change-panel">
            <p class="invoice-heading">Need to make a change?</p>
            ${booking.client_request_type
              ? `<p class="client-request-pending">${
                  booking.client_request_type === "reschedule"
                    ? `Reschedule requested for ${escapeHtml(formatBookingDate(booking.client_requested_date))} at ${escapeHtml(formatBookingTime(booking.client_requested_time))}.`
                    : "Cancellation request sent."
                }</p>`
              : `<div class="client-change-buttons">
                  <button type="button" class="client-change-btn" data-request-reschedule="${booking.id}">REQUEST RESCHEDULE</button>
                  <button type="button" class="client-change-btn danger" data-request-cancel="${booking.id}">REQUEST CANCELLATION</button>
                </div>`
            }
          </div>
        `
      ...
      ${booking.status === "completed" && !reviewedBookingIds.has(booking.id)
        ? `
          <form class="client-review-form" data-booking-id="${booking.id}">
            <p class="invoice-heading">Leave a Review</p>
            <label>Rating
              <select name="rating">
            ...
            <label>Review
              <textarea name="body" rows="3" placeholder="How was your experience?" required></textarea>
            </label>
            <button class="invoice-download-btn" type="submit">SUBMIT REVIEW</button>
            <p class="payment-proof-message" aria-live="polite"></p>
          </form>
        `
        : ""}
      ...
      invoices
        ...
        .forEach(invoice => {
          ...
          button.addEventListener("click", async () => {
            ...
            try {
              await downloadInvoicePdf(invoice, booking);
            } finally {
              button.disabled = false;
    ...
    attachBookingChangeHandlers(card, booking, user);
    attachClientReviewHandler(card, booking, user);
    list.appendChild(card);
  });
}


// ===== Dynamic public website content from SBS HQ =====

const SHOOT_KEY_BY_SLUG = {
  "portrait-birthday": "birthday",
  "studio-creative": "lifestyle",
  "graduation": "graduation",
  "maternity": "maternity",
...
function dynamicPackageObject(row) {
  return {
    label: row.name,
    price: row.price_label || money(row.price),
    photos: row.photos_received ?? "Flexible",
    edited: row.edited_photos ?? "Flexible",
    duration: row.duration_minutes ? `${row.duration_minutes} min` : "Flexible"
  };
}

function rerenderDynamicShootCatalog() {
  if (!shootCatalog) return;
  shootCatalog.innerHTML = "";

  Object.entries(shootData).forEach(([key, shoot]) => {
    if (shoot._active === false) return;

    const a = document.createElement("a");
    a.className = "catalog-card";
    a.href = `shoot.html?type=${encodeURIComponent(key)}`;
    a.innerHTML = `
      <img src="${escapeHtml(shoot.images?.[0] || "")}" alt="${escapeHtml(shoot.title)}">
      <div class="catalog-copy">
        <p class="eyebrow">${key === "custom" ? "BUILD YOUR OWN" : "SHOOT OPTION"}</p>
        <h3>${escapeHtml(shoot.title)}</h3>
        <p>${escapeHtml(shoot.description || "")}</p>
        <strong>${escapeHtml(shoot.starting === "Custom quote" || shoot.starting === "Custom" ? shoot.starting : `From ${shoot.starting}`)}</strong>
        <span>VIEW DETAILS</span>
      </div>`;
    shootCatalog.appendChild(a);
  });
...
function rerenderDynamicShootDetail() {
  if (!shootTitle) return;
  const shoot = shootData[type] || shootData.birthday;

  shootTitle.textContent = shoot.title;
  document.title = `${shoot.title} | Shot by Spaceboi`;
  const desc = document.getElementById("shootDescription");
  if (desc) desc.textContent = shoot.description || "";
  ...
  if (track && dots) {
    track.innerHTML = "";
    dots.innerHTML = "";
    const images = (shoot.images || []).slice(0,3);
    while (images.length && images.length < 3) images.push(images[0]);

    images.forEach((src, i) => {
      const img = document.createElement("img");
      img.src = src;
      img.alt = `${shoot.title} example ${i + 1}`;
  ...
  if (options) {
    options.innerHTML = "";
    (shoot.packages || []).forEach((pkg, i) => {
      const button = document.createElement("button");
      button.type = "button";
      button.className = `package-option ${i === 0 ? "active" : ""}`;
      button.innerHTML = `<span>${escapeHtml(pkg.label)}</span><strong>${escapeHtml(pkg.price)}</strong>`;
      button.addEventListener("click", () => {
        [...options.children].forEach((x,n) => x.classList.toggle("active", n === i));
        document.getElementById("summaryPhotos").textContent = pkg.photos;
        document.getElementById("summaryEdited").textContent = pkg.edited;
        document.getElementById("summaryDuration").textContent = pkg.duration;
        document.getElementById("bookShootButton").href = `booking.html?type=${encodeURIComponent(type)}&package=${i}`;
      });
      options.appendChild(button);
    });

    const first = shoot.packages?.[0];
    if (first) {
      document.getElementById("summaryPhotos").textContent = first.photos;
      document.getElementById("summaryEdited").textContent = first.edited;
      document.getElementById("summaryDuration").textContent = first.duration;
      document.getElementById("bookShootButton").href = `booking.html?type=${encodeURIComponent(type)}&package=0`;
    }
  }
}

function updateContactEverywhere(contact = {}) {
  const email = contact.email || "shotbyspaceboi@gmail.com";
  const phone = contact.whatsapp || "(876) 481-1489";
  const wa = contact.whatsapp_link || "https://wa.me/18764811489";
  const location = contact.location || "2 Majestic Pen Road";

  document.querySelectorAll(".footer-contact-line").forEach(line => {
    line.innerHTML = `<a href="mailto:${escapeHtml(email)}">${escapeHtml(email)}</a><b>•</b><a href="${escapeHtml(wa)}" target="_blank" rel="noopener">${escapeHtml(phone)}</a><b>•</b><span>${escapeHtml(location)}</span>`;
  });

  if (document.body && location.pathname.endsWith("contact.html")) {
    const cards = document.querySelectorAll(".contact-card");
    cards.forEach(card => {
      const label = card.querySelector("span")?.textContent || "";
      const strong = card.querySelector("strong");
      if (!strong) return;
      if (label.includes("EMAIL")) strong.innerHTML = `<a href="mailto:${escapeHtml(email)}">${escapeHtml(email)}</a>`;
      if (label.includes("LOCATION")) strong.textContent = location;
      if (label.includes("WHATSAPP")) strong.innerHTML = `<a href="${escapeHtml(wa)}" target="_blank" rel="noopener">${escapeHtml(phone)}</a>`;
      if (label.includes("INSTAGRAM") && contact.instagram) strong.textContent = contact.instagram;
    });
  }
}
...
async function loadDynamicPublicSite() {
  ...
  try {
    const [
      ...
      sbClient.from("shoot_types").select("id,name,slug,description,image_url,gallery_images,active,sort_order").order("sort_order"),
      sbClient.from("shoot_packages").select("*").eq("active", true).order("sort_order"),
      sbClient.from("site_content").select("key,value"),
      sbClient.from("portfolio_items").select("*").eq("published", true).order("sort_order"),
      sbClient.from("reviews").select("*").eq("approved", true).order("created_at", { ascending: false }),
      sbClient.rpc("popular_shoot_types", { p_limit: 3 })
    ]);
    ...
    types.forEach(row => {
      const key = SHOOT_KEY_BY_SLUG[row.slug];
      if (!key || !shootData[key]) return;

      const gallery = Array.isArray(row.gallery_images) && row.gallery_images.length
        ? row.gallery_images
        : (row.image_url ? [row.image_url] : shootData[key].images);

      const rows = packages.filter(pkg => pkg.shoot_type_id === row.id);

      shootData[key].title = row.name;
      shootData[key].description = row.description || shootData[key].description;
      shootData[key].images = gallery;
      shootData[key]._active = row.active;
      if (rows.length) {
        shootData[key].packages = rows.map(dynamicPackageObject);
        shootData[key].starting = rows[0].price_label || money(rows[0].price);
      }
    });

    rerenderDynamicShootCatalog();
    rerenderDynamicShootDetail();

    const hero = content.hero || {};
    const heroEl = document.querySelector(".home-hero");
    if (heroEl && hero.image_url) {
      heroEl.style.backgroundImage = `url("${hero.image_url}")`;
    }

    const about = content.about || {};
    const homeAboutPhoto = document.querySelector(".about-home-photo img");
    if (homeAboutPhoto && about.image_url) homeAboutPhoto.src = about.image_url;
    const homeAboutHeading = document.querySelector(".about-home-copy h2");
    if (homeAboutHeading && about.heading) homeAboutHeading.textContent = about.heading;
    const homeAboutBody = document.querySelector(".about-home-copy p:not(.eyebrow)");
    if (homeAboutBody && about.body) homeAboutBody.textContent = about.body;

    const fullAboutPhoto = document.querySelector(".about-full-photo img");
    if (fullAboutPhoto && about.image_url) fullAboutPhoto.src = about.image_url;
    const fullAboutHeading = document.querySelector(".about-full-copy h1");
    if (fullAboutHeading && about.heading) fullAboutHeading.textContent = about.heading;
    const fullAboutParagraphs = document.querySelectorAll(".about-full-copy p:not(.eyebrow)");
    if (fullAboutParagraphs.length && about.body) {
      fullAboutParagraphs[0].textContent = about.body;
      if (fullAboutParagraphs[1]) fullAboutParagraphs[1].style.display = "none";
    }

    updateContactEverywhere(content.contact || {});

    const portfolio = portfolioResult.data || [];
    const preview = document.querySelector(".preview-grid");
    if (preview && portfolio.length) {
      preview.innerHTML = portfolio.slice(0,4).map(item => `<img src="${escapeHtml(item.image_url)}" alt="${escapeHtml(item.caption || "Shot by Spaceboi portfolio")}">`).join("");
    }
    const fullPortfolio = document.querySelector(".portfolio-full");
    if (fullPortfolio && portfolio.length) {
      fullPortfolio.innerHTML = portfolio.map(item => `<img src="${escapeHtml(item.image_url)}" alt="${escapeHtml(item.caption || "Shot by Spaceboi portfolio")}">`).join("");
    }

    const reviews = reviewsResult.data || [];
    const reviewsEl = document.querySelector(".reviews");
    if (reviewsEl && reviews.length) {
      reviewsEl.innerHTML = reviews.slice(0,6).map(review => `<article><div>${"★".repeat(review.rating)}${"☆".repeat(5-review.rating)}</div><h3>${escapeHtml(review.client_name)}</h3><p>“${escapeHtml(review.body)}”</p></article>`).join("");
    }

    const popular = popularResult.data || [];
    const popularEl = document.getElementById("popularShoots");
    if (popularEl && popular.length) {
      popularEl.innerHTML = popular.map(row => {
        const key = SHOOT_KEY_BY_SLUG[row.slug];
        const shoot = shootData[key];
        if (!shoot) return "";
        return `<a class="shoot-card" href="shoot.html?type=${encodeURIComponent(key)}">
          <img src="${escapeHtml(shoot.images?.[0] || "")}" alt="${escapeHtml(shoot.title)}">
          <div class="card-copy"><p class="eyebrow light">POPULAR</p><h3>${escapeHtml(shoot.title)}</h3><p>${escapeHtml(shoot.description || "")}</p><span>VIEW DETAILS →</span></div>
        </a>`;
      }).join("");
    }
  } catch (error) {
